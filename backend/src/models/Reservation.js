...
export const createReservation = async (db, { user_id, vehicle_id, start_date, end_date, motif }) => {
  const stmt = await db.prepare(
    'INSERT INTO reservations (user_id, vehicule_id, start_date, end_date, motif) VALUES (?, ?, ?, ?, ?)'
  );
  const result = await stmt.run(user_id, vehicle_id, start_date, end_date, motif);
  return { id: result.lastID, user_id, vehicle_id, start_date, end_date, status: 'pending', motif };
};
...
export const cancelReservation = async (db, reservation_id, user_id, isAdmin = false) => {
  const stmt = await db.prepare(`
    UPDATE reservations
    SET status = 'cancelled' WHERE id = ? ${isAdmin ? '' : 'AND user_id = ?'} AND status = 'confirmed'
  `);
  const result = await stmt.run(reservation_id, ...(isAdmin ? [] : [user_id]));
  return result.changes > 0; // Retourne true si une ligne a été modifiée
};

// Approuver une réservation (admin seulement)
export const approveReservation = async (db, reservation_id) => {
  const stmt = await db.prepare(`
    UPDATE reservations
    SET status = 'confirmed' WHERE id = ? AND status = 'pending'
  `);
  const result = await stmt.run(reservation_id);
  return result.changes > 0; // Retourne true si une ligne a été modifiée
};

// Désapprouver une réservation (admin seulement)
export const disapproveReservation = async (db, reservation_id) => {
  const stmt = await db.prepare(`
    UPDATE reservations
...
export const getReservationsWithDetails = async (db) => {
  const stmt = await db.prepare(`
    SELECT
      r.id, r.user_id, r.vehicule_id, r.start_date, r.end_date, r.status, r.motif, r.created_at,
